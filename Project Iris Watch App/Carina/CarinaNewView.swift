...
"""
  ...
  @State var sendHistoryFile = false
  @State var sendBookmarksFile = false
  ...
  @State var historyLibrary = "placeholder"
  @State var bookmarksLibrary = "placeholder"
    ...
    if sent {
      if CarinaID == -1 || CarinaID == -2 {
        ...
        VStack {
          ...
          Text("Carina.sending")
          Group {
            Text(carinaTypes[type] ?? "Carina.type.other") + Text(verbatim: " · ") + Text(carinaPlaces[place] ?? "Carina.place.other") + Text(verbatim: " · ") + Text(verbatim: "Iris \(Bundle.main.infoDictionary?["CFBundleShortVersionString"] as! String)")
          }
          .foregroundStyle(.secondary)
        }
        .multilineTextAlignment(.center)
      ...
      List {
        ...
        NavigationLink(destination: {
          List {
            ...
            Section(content: {
              Toggle(isOn: $sendBookmarksFile, label: {
                Text("Carina.new.attaching-files.bookmarks")
              })
              Toggle(isOn: $sendHistoryFile, label: {
                Text("Carina.new.attaching-files.history")
              })
            }, header: {
              Text("Carina.new.attaching-files.title")
            }, footer: {
              Text("Carina.new.attaching-files.description")
            })
            ...
            Section {
              ...
              .sheet(isPresented: $showWhatsIncludedSheet, content: {
                CarinaAttachmentItemsView(sendSettingsValues: sendAllSettingsValue, sendDeviceInformations: sendDeviceInformations, sendLocaleInformations: sendLocaleInformations, sendBookmarksFile: sendBookmarksFile, sendHistoryFile: sendHistoryFile)
              })
            }
          }
          .navigationTitle("Carina.new.attachments")
          .onDisappear {
            attachmentCounts = (sendAllSettingsValue ? 1 : 0) + (sendDeviceInformations ? 1 : 0) + (sendLocaleInformations ? 1 : 0) + (sendBookmarksFile ? 1 : 0) + (sendHistoryFile ? 1 : 0) + attachmentLinks.count
          }
        }, label: {
          HStack {
      ...
      .onAppear {
        ...
        bookmarksLibrary = readPlainTextFile("BookmarkLibrary.txt")
        historyLibrary = readPlainTextFile("historyData.txt")
...
""")
    ...
    if sendBookmarksFile {
      package.append("""

BookmarksFile：\(bookmarksLibrary)
""")
    }
    if sendHistoryFile {
      package.append("""

HistoryFile：\(historyLibrary)
""")
    }
...
struct CarinaAttachmentItemsView: View {
  ...
  var sendBookmarksFile: Bool
  var sendHistoryFile: Bool
  var body: some View {
    ScrollView {
      VStack(alignment: .leading) {
        ...
        Group {
          Section(content: {
            ...
            CarinaAttachmentSingleItemView(text: "Carina.new.attachments.items.settings.bookmark", image: "bookmark", send: sendBookmarksFile)
            CarinaAttachmentSingleItemView(text: "Carina.new.attachments.items.settings.history", image: "clock.arrow.circlepath", send: sendHistoryFile)
