...
struct BookmarksView: View {
  ...
  var body: some View {
    ...
    .navigationTitle("Home.bookmarks")
    .toolbar {
      if #available(watchOS 10.0, *) {
        ToolbarItemGroup(placement: .bottomBar) {
            ...
            if isBookmarkCounterDisplayed {
              Label("\(bookmarkLinks.count)", systemImage: "bookmark")
                .accessibilityLabel(String(localized: "Bookmark.count.\(bookmarkLinks.count)"))
...
//                  .mask(Rectangle().background(.ultraThickMaterial))
    ...
    .sheet(isPresented: $isNewBookmarkPresenting, content: {
      NavigationStack {
        List {
          CepheusKeyboard(input: $bookmarkTitle, prompt: "Bookmark.title", style: "field-page")
          CepheusKeyboard(input: $bookmarkLink, prompt: "Bookmark.link", style: "field-page", autoCorrectionIsEnabled: false)
          Button(action: {
